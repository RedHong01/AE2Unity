...
```text
https://github.com/RedHong01/duo-curtain-ae2unityshader.git
```

You can also add it directly to `Packages/manifest.json`:

```json
"com.duocurtain.ae2unityshader": "https://github.com/RedHong01/duo-curtain-ae2unityshader.git"
```
...
1. Run `Tools/AfterEffects/AE2UnityShaderExport.jsx` in After Effects.
...
7. Unity's AEBridge receiver imports the payload into `Assets/AE2Unity/Exports/<CompName>.ae2shader`.
...
Direct export and manual generation are still available through `Direct .ae2shader into Unity Assets`, `Manual folder .ae2shader export`, and `Assets > Duo Curtain AE2Unity > Generate Shader From AE2Shader`.
...
- Unity material preview through `Duo Curtain/AE2Unity/Composite Unlit`.
...
Open `Project Settings > Duo Curtain > AE2Unity Shader`.
...
Bridge utilities are also available in `Tools > Duo Curtain AE2Unity`.
