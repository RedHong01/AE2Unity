...
3. 选择 `Packages/com.redhong01.ae2unityshader/Tools/AfterEffects/ae2unityshader.jsx`。
...
重启 AE 后，从 `Window` 菜单打开 `ae2unityshader`。
...
```bash
cd Packages/com.redhong01.ae2unityshader/Tools/AfterEffects
...
6. `.ae2shader Folder` 默认是 `Assets/ae2unityshader/Exports`，可以按项目需要改。
...
```text
Assets/ae2unityshader/Exports/<CompName>.ae2shader
```

然后自动生成：

```text
Assets/ae2unityshader/Exports/<CompName>.generated.shader
Assets/ae2unityshader/Exports/<CompName>.generated.mat
```
...
- 把选定 Composition 加入 AE Render Queue，并调用 `queueInAME()` 发送到 Adobe Media Encoder。媒体文件目标路径会落在 Unity 项目内的 `Media Folder`，默认是 `Assets/ae2unityshader/Media`。
...
```text
Project Settings > ae2unityshader
```
...
```text
Tools > ae2unityshader > Process AEBridge Inbox Now
Tools > ae2unityshader > Open AEBridge Folder
```
...
这会直接把 `.ae2shader` 写进 Unity 的 `Assets/ae2unityshader/Exports`，再由普通 Asset Importer 自动生成。
