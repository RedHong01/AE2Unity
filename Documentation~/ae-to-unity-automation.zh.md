...
3. 选择 `Packages/com.duocurtain.ae2unityshader/Tools/AfterEffects/AE2UnityShaderExport.jsx`。
...
重启 AE 后，从 `Window` 菜单打开 `AE2Unity Shader Export`。
...
```bash
cd Packages/com.duocurtain.ae2unityshader/Tools/AfterEffects
...
1. 打开 `AE2Unity Shader Export` 面板。
...
```text
Project Settings > Duo Curtain > AE2Unity Shader
```
...
```text
Tools > Duo Curtain AE2Unity > Process AEBridge Inbox Now
Tools > Duo Curtain AE2Unity > Open AEBridge Folder
```
